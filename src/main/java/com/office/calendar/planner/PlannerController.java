...
import com.office.calendar.planner.util.S3UploadFileService;
import com.office.calendar.planner.util.UploadedFileInfo;
...
public class PlannerController {
    ...
    final private S3UploadFileService s3UploadFileService;
    ...
    public ResponseEntity<Map<String, Object>> writePlan(PlannerDto plannerDto,
        ...
        // SAVE FILE
        UploadedFileInfo uploadedFileInfo = s3UploadFileService.upload(file);
        if (uploadedFileInfo != null) {
            plannerDto.setImg_name(uploadedFileInfo.getFileName());
    ...
    public ResponseEntity<Map<String, Object>> modifyPlan(
        ...
        plannerDto.setNo(no);
        if(file != null) {
            UploadedFileInfo savedFileName = s3UploadFileService.upload(file);
            if(savedFileName != null) {
                plannerDto.setOwner_id(principal.getName());
                plannerDto.setImg_name(savedFileName.getFileName());
